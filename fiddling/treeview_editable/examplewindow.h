...
protected:
  ...
  public:
    ...
    { add(m_col_id); add(m_col_name); add(m_col_foo); add(m_col_number);
      add(m_col_number_validated); add(m_col_editable); } //!!
  ...
  //!! Unless Bug 659672 "gtk_tree_view_column_dispose() does not remove from tree view"
  // has been fixed in the used version of gtk+, delete m_TreeView before
  // m_treeviewcolumn_validated, or else lots of critical messages will be
  // printed when the program's window is closed.
  Gtk::TreeView m_TreeView; //!!
};
