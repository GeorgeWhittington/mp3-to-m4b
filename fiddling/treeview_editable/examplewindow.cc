  ...
  m_validate_retry(false)
{
  ...
  //!!Column m_col_editable decides in which rows m_col_name and m_col_number_validated are editable.
  Gtk::TreeViewColumn* col_name = m_TreeView.get_column(1);
  Gtk::CellRenderer* cellrenderer_name = m_TreeView.get_column_cell_renderer(1);
  col_name->add_attribute(*cellrenderer_name, "editable", m_Columns.m_col_editable);

  Gtk::TreeViewColumn* col_validated = m_TreeView.get_column(4);
  col_validated->add_attribute(m_cellrenderer_validated, "editable", m_Columns.m_col_editable);
  //!! 
